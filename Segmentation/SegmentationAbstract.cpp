...
//trace.error()<<"binWidth:"<<binWidth<<std::endl;
    ...
    int nbInterval = range / res + 1;


std::cout<< nbInterval<<std::endl;
...
double SegmentationAbstract::getThresholdByRosin(const std::vector<double> &data, double binSize){
    if(data.size() == 0){
        return 0;
    }
    //build histogram
    ...
    double range = maxValue - minValue;
    int nbInterval = (int)std::ceil(range / binSize);
...
std::vector<unsigned int> 
SegmentationAbstract::getNegativeDefect(double threshold){
    std::vector<unsigned int> negDefects;
    for(unsigned int i = 0; i < myPoints.size(); i++){
        if(distances[i] < threshold){
            negDefects.push_back(i);
        }
    }
    return negDefects;
}
...
std::vector<unsigned int> 
SegmentationAbstract::getNegativeDefect(){
    std::vector<double> negDistances(distances.size());
    for(int i = 0; i < negDistances.size(); i++){
        negDistances[i] = -distances[i];
    }
    double th = -getThresholdByRosin(negDistances, binWidth);
    return getNegativeDefect(th);
}
...
void SegmentationAbstract::convertToCcs(){
    //double sumRadii = 0.0;
    std::vector<double> radiis(pointCloud.size());
    ...
    for(unsigned int i = 0; i < pointCloud.size(); i++){
        ...
        assert(segmentId < centerline.size() - 1);

        Z3i::RealPoint aDir = centerline[segmentId + 1] - centerline[segmentId];
        aDir = aDir / aDir.norm();
        Z3i::RealPoint vectRadial = getRadialVector(pointCloud[i], aDir, centerline[segmentId]);
        ...
        Z3i::RealPoint p0 = centerline[segmentId];
    ...
    radii = Statistic::getMode(radiis, minR, maxR, 1);
        ///sumRadii / pointCloud.size();
}
...
SegmentationAbstract::computeBeginOfSegment(){
    ...
    for (int i = 1; i < nbSegment; i++){
        Z3i::RealPoint vectDir = centerline.at(i) - centerline.at(i - 1);
...
SegmentationAbstract::computeVectorMarks(){
    ...
    for (unsigned int i = 0; i < centerline.size() - 1; i++){
        ...
        Z3i::RealPoint p0 = centerline.at(i);
