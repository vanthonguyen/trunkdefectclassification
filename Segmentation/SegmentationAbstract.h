...
class SegmentationAbstract{
    public:
        SegmentationAbstract(std::vector<Z3i::RealPoint> &aCloud, std::vector<Z3i::RealPoint> &aFib, double arcLen, int wh, double bw):
                       pointCloud(aCloud), centerline(aFib), arcLength(arcLen), patchHeight(wh), binWidth(bw){
        }

        virtual void init() = 0;
        virtual std::vector<unsigned int> getDefect();
        //Defects with delta < 0
        std::vector<unsigned int> getNegativeDefect();
        std::vector<unsigned int> getNegativeDefect(double threshold);
        ...
        double getThresholdByRosin(const std::vector<double> &data, double binSize);
