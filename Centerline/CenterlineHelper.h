...
public:  
    ...
    computeCenterline(const std::vector<Z3i::RealPoint> &points, const double &voxelSize,
            double searchRadius, double accRadius, const int &nbControlPoint, const int &nbSegment=1, double confThreshold = 0.0) {
        int nbPoints = points.size();
        //adjust accumulation parameters by voxel size
        searchRadius /= voxelSize;
        accRadius /= voxelSize;

        std::vector<Z3i::RealPoint> centers(nbPoints);
        std::vector<Z3i::RealPoint> normals(nbPoints);

        Z3i::RealPoint lowPt = points.at(0) / voxelSize;
        Z3i::RealPoint upPt = points.at(0) / voxelSize;

        pcl::PointCloud<pcl::PointXYZ>::Ptr cloud (new pcl::PointCloud<pcl::PointXYZ>);

        //adjust by voxelSize
        //compute normal vector + center points
        for ( int i = 0; i < points.size(); i++ ){
            Z3i::RealPoint cp = points[i] / voxelSize;
            centers[i] = cp;
            lowPt = lowPt.inf(cp);
            upPt = upPt.sup(cp);

            pcl::PointXYZ pxyz(cp[0], cp[1], cp[2]);
            cloud->points.push_back(pxyz);
        }


        std::cout<<"Begin estimate normal by Pcl"<<std::endl;
        ...
        for ( int i = 0; i < cloudNormals->points.size(); i++ ){
            pcl::Normal pn = cloudNormals->points[i];
            normals[i] = Z3i::RealPoint(pn.normal_x, pn.normal_y, pn.normal_z);
        }

        assert(centers.size() == normals.size());       

        //Centerline cen(accRadius, myDomain, normals, centers, th, nbControlPoint);
        CenterlineMultiCore cen(accRadius, normals, centers, nbControlPoint, nbSegment, confThreshold);

        //init accumulator
        std::vector<Z3i::RealPoint> smoothLine = cen.compute();

        //restore scaled line
        for( int i = 0; i < smoothLine.size(); i++ ){
            Z3i::RealPoint& p = smoothLine[i];
            p *= voxelSize;
        }

        //IOHelper::export2Text(smoothLine, "centerline.xyz");
        return smoothLine;
    }
